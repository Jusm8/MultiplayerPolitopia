...
@onready var botton_panel: PanelContainer = $TopBar/BottonPanel
@onready var label_title_info: Label = $TopBar/PanelContainer/HBoxContainer/LabelTitleInfo
@onready var actions_container: HBoxContainer = $TopBar/BottonPanel/HBoxContainer/ActionsContainer
...
func _ready() -> void:
	...
	end_turn_btn.pressed.connect(_on_end_turn_botton_pressed)
	confirm_end_turn_dialog.confirmed.connect(_on_confirmed)

func _on_end_turn_botton_pressed() -> void:
...
func  set_selected_tile(cell: Vector2i, terrain: int) -> void:
	...
	match terrain:
		...
		3: 
			terrain_name = "Agua"
		4: 
			terrain_name = "Montaña"
		_:
			terrain_name = "Desconicido"
